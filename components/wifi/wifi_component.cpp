...
void WiFiComponent::clear_stored_creds() {
  ...
  // create new preferences object with different hash from the one used to save valid credentials
  this->pref_ = global_preferences->make_preference<wifi::SavedWifiSettings>(0, true);

  // save blank credentials with different hash so when we load them later there is nothing there.
...
void WiFiComponent::setup() {
  ...
  // hash the first public build datetime so that all firmware versions always have the same hash
  uint32_t hash = fnv1_hash("Aug 16 2021, 20:24:52");
