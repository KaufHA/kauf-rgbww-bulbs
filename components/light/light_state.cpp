...
void LightState::restore_with_mode(uint32_t transition_length) {
  ...
  this->remote_values.set_brightness(recovered.brightness);
  this->remote_values.set_color_brightness(recovered.color_brightness);
  this->remote_values.set_red(recovered.red);
  this->remote_values.set_green(recovered.green);
  this->remote_values.set_blue(recovered.blue);
  this->remote_values.set_white(recovered.white);
  this->remote_values.set_color_temperature(recovered.color_temp);
  this->remote_values.set_cold_white(recovered.cold_white);
  this->remote_values.set_warm_white(recovered.warm_white);
  ...
  if (recovered.effect != 0) {
    ...
    call.set_transition_length_if_supported(transition_length); // KAUF: default 0
  }
  call.set_save(false); // KAUF: restoring saved values, don't need to re-save
  call.perform();
}
...
void LightState::loop() {
  ...
  if ( !this->output_->is_aux() ) {
    if ( (this->output_->warm_rgb != nullptr && this->output_->warm_rgb->has_changed) ||
         (this->output_->cold_rgb != nullptr && this->output_->cold_rgb->has_changed) ) {
      ESP_LOGV("KAUF_OUTPUT","warm or cold rgb changed");
      if (this->output_->warm_rgb != nullptr) this->output_->warm_rgb->has_changed = false;
      if (this->output_->cold_rgb != nullptr) this->output_->cold_rgb->has_changed = false;
      this->next_write_ = true;
    }
  }
#endif
...
bool LightState::parse_frame_(const uint8_t *payload, uint16_t size) {
  ...
  if ( this->remote_values.is_on() && (max != 0.0f) ) {
    ...
    const float scale = this->remote_values.get_brightness() * (1.0f / max);
    scaled_r = r * scale;
    scaled_g = g * scale;
    scaled_b = b * scale;
  } else {
