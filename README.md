...
***Warm RGB*** light entity - Disabled by default.  Allows the definition of an RGB value which will be mixed in with the warm white LEDs when the bulb is in color temperature mode.  This entity is not meant to directly control the bulb.
...
***Power On State*** select entity - Sets the behavior of the bulb on boot / restoration of power.  Defaults to `Always On - Last Value`
- `Restore Power Off State`: Same as ESPHome's `RESTORE_DEFAULT_OFF`.  The bulb will attempt to restore the most recent value of the main light entity and stay off if it can't restore.
- `Always On - Last Value`: Same as ESPHome's `RESTORE_AND_ON`.  The bulb will always turn on at boot and restore its most recent value if possible.  If it can't be restored, the bulb will still turn on but to the default bright white.
...
***No HASS*** switch entity - When turned on, the bulb assumes that it is not being used with Home Assistant and disables the reboot timeout.
...
***name*** - The name is used to name the device for mDNS, the ESPHome dashboard, and in Home Assistant.
...
***wifi_ap_timeout*** - Defines the amount of time after a Wi-Fi connection is lost that the bulb will put up its Wi-Fi AP to allow reconfiguration of credentials.  This defaults to 15 seconds on the precompiled binaries but 2 minutes on the yaml package.
...
***sub_reboot_req*** - The number of reboots in a row, each occurring after less than 10 seconds or without the bulb connecting to Wi-Fi, required to reset the stored Wi-Fi credentials and force-enable the Wi-Fi AP.  Defaults to 9.
